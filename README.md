...
- 「売上詳細」は通常の売上ランキングを表示し、選択中にもう一度押すとサービス別売上内訳を表示します。
- 売上詳細表は同じ期間・エリア条件のランキングデータから、順位、名前、売上、Uber、ロケナウ、出前館、menu、Other、稼働時間、時給、件数、最大単価を表示します。
- 売上詳細表は表の中だけ横スクロールでき、ページ全体は横スクロールしない設計です。
