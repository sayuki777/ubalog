...
- 記録画面は開始・終了・休憩を直接入力し、保存前に売上・件数・稼働時間の確認を表示します。
...
- カレンダーは目標を青、売上を緑で表示し、目標状況をカレンダー下にコンパクト表示します。
- OGP画像は `public/ogp.png` を使い、SNS向けmetadataでは `https://ubalog.vercel.app/ogp.png` を参照します。
- 記録タブ下部の個人成績枠は初期表示で開き、マイページと同じ records/goals を使って個人成績、月間売上、目標を表示します。
- 目標カレンダーのセル内数字はカンマなし表示にし、目標状況は週をメイン、月を下に表示します。
- ランキングの売上/詳細テーブルから、表示中の順位順データをBOM付きUTF-8 CSVで出力できます。
- 記録タブ上部はサマリー枠を外し、goals/records共有の目標売上カレンダーを配置しています。
- 上部カレンダーは今日をハイライトし、目標表示のON/OFFを切り替えできます。
- 上部カレンダーの日付をタップすると、下の目標入力カードと記録フォームの日付が同じ日に切り替わります。
- 記録タブ上部カレンダーの目標ON/OFFはカレンダー下に配置し、上部の日別入力・指標カードは表示しません。
- 記録タブの昨日コピーと稼働時間テンプレート行は表示せず、開始・終了・休憩入力を直接使う構成です。
- ランキングカードは1〜3位も4位基準のコンパクトな縦幅に寄せ、色や太字で順位差を出します。
